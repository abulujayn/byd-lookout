 ...
 * ~256 bytes each at 20 ms cadence (~50 frames per second). A 5-second window
 * is therefore ~250 packets and ~64 KB of payload — small enough that a
 * lock-free deque + per-packet heap copies are cheaper and easier to reason
 * about than the seqlock-validated direct-byte arena the video ring needs.
 *
 * <h3>Concurrency model</h3>
 * Single producer (the daemon's audio ingest thread, calling
 * {@link #add(byte[], int, int, long)}) plus a single consumer (the
 * trigger/event thread, calling {@link #drainAll()}). {@link #clear()} is
 * called from start-up and audio-disable paths. All three are wait-free apart
 * from {@link ConcurrentLinkedDeque}'s internal CAS retries.
 *
 * <h3>Eviction</h3>
 ...
 * land slightly above the bitrate-time product. At 5 s × 64 kbps that's
 * 5 × 64000 / 8 × 1.5 = 60 000 bytes — under 60 KB, negligible vs. the video
 * ring's 64 MB.
 *
 ...
 *       a 60 KB budget at 256 B/packet caps the deque at ~250 entries.</li>
 * </ul>
 */
public class AacCircularBuffer {
     ...
     *                           seconds. Clamped to [1, 30] to mirror
    ...
    public AacCircularBuffer(int maxDurationSeconds, int maxBitrateBps) {
        int clampedSec = Math.max(1, Math.min(30, maxDurationSeconds));
    ...
    public List<Packet> drainAll() {
        ...
        // drainedBytes is bounded by byteBudget (~60 KB) so the cast to int
        // is safe — Math.toIntExact() would also work but adds an exception
        // path for an arithmetically impossible overflow.
        if (drainedBytes > 0) {
     ...
     *          ring is small — ~250 entries at 5 s × 64 kbps). Diagnostic
     *          only. */
